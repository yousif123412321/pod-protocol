...
export class EscrowCommands {
  ...
  private async handleDeposit(client: PodComClient, wallet: any, globalOpts: GlobalOptions, options: any) {
  ...
  private async handleWithdraw(client: PodComClient, wallet: any, globalOpts: GlobalOptions, options: any) {
    let channelId = options.channel;
    let amount: number = 0;
  ...
  private async handleBalance(client: PodComClient, wallet: any, options: any) {
    ...
    const data = [
      ...
      ["Balance", formatValue(`${lamportsToSol(escrowData.balance)} SOL`, 'number')],
      ["Balance (Lamports)", formatValue(escrowData.balance.toString(), 'number')],
      [
        "Last Updated",
