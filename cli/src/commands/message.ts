...
import { PodComClient, MessageType, MessageStatus, getMessageTypeId } from "@pod-com/sdk";
...
export class MessageCommands {
  register(program: Command) {
    ...
    message
      ...
      .option("-t, --type <type>", "Message type (text, data, command, response, custom)", "text")
      .option("-c, --custom-value <number>", "Custom value for custom message types")
      ...
      .action(async (options, cmd) => {
        ...
        try {
          ...
          let customValue = options.customValue ? parseInt(options.customValue) : 0;
          ...
          if (options.interactive) {
            const answers = await inquirer.prompt([
              ...
              },
              {
                ...
                choices: [
                  { name: "Text - Plain text message", value: MessageType.Text },
                  { name: "Data - Structured data transfer", value: MessageType.Data },
                  { name: "Command - Command/instruction", value: MessageType.Command },
                  { name: "Response - Response to command", value: MessageType.Response },
                  { name: "Custom - Custom message type", value: MessageType.Custom }
                ]
              },
              {
                type: "editor",
          ...
          if (globalOpts.dryRun) {
            ...
            console.log(chalk.cyan("Payload:"), payload.slice(0, 100) + (payload.length > 100 ? "..." : ""));
            return;
          }
      ...
      .action(async (messageId, cmd) => {
        ...
        try {
          ...
          const data = [
            ...
            ["Payload", messageData.payload.slice(0, 100) + (messageData.payload.length > 100 ? "..." : "")],
            ["Timestamp", new Date(messageData.timestamp * 1000).toLocaleString()],
            ["Expires At", messageData.expiresAt ? new Date(messageData.expiresAt * 1000).toLocaleString() : "Never"]
          ];
    ...
    message
      ...
      .option("-s, --status <status>", "New status (pending, delivered, read, failed)")
      .action(async (options, cmd) => {
        const globalOpts = cmd.optsWithGlobals();
        ...
        try {
          if (!options.message || !options.status) {
            ...
            console.log(chalk.yellow("Usage: pod message status -m <messageId> -s <status>"));
            return;
          }

          const validStatuses = ["pending", "delivered", "read", "failed"];
          if (!validStatuses.includes(options.status)) {
            console.error(chalk.red("Error: Invalid status. Must be one of:"), validStatuses.join(", "));
            return;
          }
          ...
          console.error(chalk.red("Failed to update message status:"), error.message);
          process.exit(1);
        }
      });
    ...
    message
      ...
      .option("-l, --limit <number>", "Maximum number of messages to show", "10")
      .option("-f, --filter <status>", "Filter by status (pending, delivered, read, failed)")
      .action(async (options, cmd) => {
        const globalOpts = cmd.optsWithGlobals();
