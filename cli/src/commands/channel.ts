...
export class ChannelCommands {
  register(program: Command) {
    ...
    channel
      ...
      .action(createCommandHandler("broadcast message", async (client, wallet, globalOpts, channelId, message, options) => {
        await this.handleBroadcast(client, wallet, globalOpts, channelId, message, options);
      }));
    ...
    channel
      ...
      .action(createCommandHandler("send invitation", async (client, wallet, globalOpts, channelId, invitee) => {
        await this.handleInvite(client, wallet, globalOpts, channelId, invitee);
      }));
  ...
  private async handleCreate(client: PodComClient, wallet: any, globalOpts: GlobalOptions, options: any) {
    ...
    const validatedMaxParticipants = validatePositiveInteger(maxParticipants, "max participants");
    const validatedFeePerMessage = validatePositiveInteger(feePerMessage, "fee per message");

    const spinner = createSpinner("Creating channel...");

    if (handleDryRun(globalOpts, spinner, "Channel creation", {
      "Name": validatedName,
  ...
  private async handleInfo(client: PodComClient, channelId: string) {
    ...
    const data = [
      ...
      ["Description", formatValue(channelData.description || "No description", 'text')],
      ["Creator", formatValue(channelData.creator.toBase58(), 'address')],
      ["Visibility", formatValue(channelData.visibility, 'text')],
      [
        "Participants",
        formatValue(`${channelData.participantCount}/${channelData.maxParticipants}`, 'text'),
      ],
      ["Fee per Message", formatValue(`${channelData.feePerMessage} lamports`, 'number')],
      [
        "Created",
        formatValue(new Date(channelData.createdAt * 1000).toLocaleString(), 'text'),
      ],
  ...
  private async handleList(client: PodComClient, wallet: any, options: any) {
    ...
    const data = channels.map((ch: any) => [
      ...
      formatValue(ch.name.slice(0, 20) + (ch.name.length > 20 ? "..." : ""), 'text'),
  ...
  private async handleJoin(client: PodComClient, wallet: any, globalOpts: GlobalOptions, channelId: string) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const spinner = createSpinner("Joining channel...");

    if (handleDryRun(globalOpts, spinner, "Channel join", {
      "Channel": channelId
    })) {
      return;
    }
  ...
  private async handleLeave(client: PodComClient, wallet: any, globalOpts: GlobalOptions, channelId: string) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const spinner = createSpinner("Leaving channel...");

    if (handleDryRun(globalOpts, spinner, "Channel leave", {
      "Channel": channelId
    })) {
      return;
    }
  ...
  private async handleBroadcast(client: PodComClient, wallet: any, globalOpts: GlobalOptions, channelId: string, message: string, options: any) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const validatedMessage = validateMessage(message);
    const spinner = createSpinner("Broadcasting message...");

    if (handleDryRun(globalOpts, spinner, "Message broadcast", {
      "Channel": channelId,
      "Message": validatedMessage.slice(0, 50) + (validatedMessage.length > 50 ? "..." : ""),
      "Type": options.type
    })) {
      return;
    }
    ...
    const signature = await client.broadcastMessage(
      ...
      validatedMessage,
      options.type as any,
      options.replyTo ? validatePublicKey(options.replyTo, "reply-to message ID") : undefined,
    );
  ...
  private async handleInvite(client: PodComClient, wallet: any, globalOpts: GlobalOptions, channelId: string, invitee: string) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const inviteeKey = validatePublicKey(invitee, "invitee address");
    const spinner = createSpinner("Sending invitation...");

    if (handleDryRun(globalOpts, spinner, "Invitation", {
      "Channel": channelId,
      "Invitee": invitee
    })) {
      return;
  ...
  private async handleParticipants(client: PodComClient, channelId: string, options: any) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const limit = validatePositiveInteger(options.limit, "limit");
  ...
  private async handleMessages(client: PodComClient, channelId: string, options: any) {
    const channelKey = validatePublicKey(channelId, "channel ID");
    const limit = validatePositiveInteger(options.limit, "limit");
    ...
    const data = messages.map((m: any) => [
      ...
      formatValue(m.content.slice(0, 50) + (m.content.length > 50 ? "..." : ""), 'text'),
