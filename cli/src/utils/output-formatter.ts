...
import { BRAND_COLORS, ICONS, keyValue, sectionHeader, DIVIDERS } from "./branding.js";
...
export class OutputFormatter {
  ...
  public displayAgentInfo(agent: any): void {
    ...
    console.log(keyValue('Address', agent.pubkey?.toString() || 'Unknown', ICONS.key));
    console.log(keyValue('Capabilities', this.formatCapabilities(agent.capabilities), ICONS.gear));
    console.log(keyValue('Metadata URI', agent.metadataUri || 'None', ICONS.info));
    console.log(keyValue('Reputation', agent.reputation || 0, ICONS.star));
    console.log(keyValue('Last Updated', this.formatTimestamp(agent.lastUpdated), ICONS.bell));
  ...
  public displayMessageInfo(message: any): void {
    ...
    console.log(keyValue('From', message.sender?.toString() || 'Unknown', ICONS.agent));
    console.log(keyValue('To', message.recipient?.toString() || 'Unknown', ICONS.agent));
    console.log(keyValue('Type', this.formatMessageType(message.messageType), ICONS.info));
    console.log(keyValue('Status', this.formatMessageStatus(message.status), ICONS.bell));
    console.log(keyValue('Timestamp', this.formatTimestamp(message.timestamp), ICONS.bell));
  ...
  public displayChannelInfo(channel: any): void {
    ...
    console.log(keyValue('Description', channel.description || 'None', ICONS.info));
    console.log(keyValue('Creator', channel.creator?.toString() || 'Unknown', ICONS.agent));
    console.log(keyValue('Visibility', this.formatVisibility(channel.visibility), ICONS.shield));
    console.log(keyValue('Participants', `${channel.participantCount || 0}/${channel.maxParticipants || 'Unlimited'}`, ICONS.agent));
    console.log(keyValue('Fee per Message', this.formatSol(channel.feePerMessage), ICONS.escrow));
  ...
  public displayEscrowInfo(escrow: any): void {
    ...
    console.log(keyValue('Channel', escrow.channel?.toString() || 'Unknown', ICONS.channel));
    console.log(keyValue('Depositor', escrow.depositor?.toString() || 'Unknown', ICONS.agent));
    console.log(keyValue('Balance', this.formatSol(escrow.balance), ICONS.escrow));
    console.log(keyValue('Last Deposit', this.formatTimestamp(escrow.lastDeposit), ICONS.bell));
   ...
   */
  public displaySummary(title: string, items: { label: string; value: string; icon?: string }[]): void {
    if (this.quiet) return;
    ...
    items.forEach(item => {
      ...
      console.log(`  ${icon}${BRAND_COLORS.accent(item.label)}:${padding}${BRAND_COLORS.secondary(item.value)}`);
    });
   ...
   */
  public displayCommandHelp(command: string, description: string, examples: { command: string; description: string }[]): void {
    ...
    if (examples.length > 0) {
      ...
      examples.forEach(example => {
        console.log(`  ${BRAND_COLORS.muted('$')} ${BRAND_COLORS.accent(example.command)}`);
  ...
  private formatMessageStatus(status: any): string {
    ...
    const mapped = statusMap[status] || { text: String(status), color: BRAND_COLORS.muted };
