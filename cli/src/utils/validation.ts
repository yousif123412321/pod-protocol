 ...
 */
export function validatePublicKey(address: string, fieldName: string = "address"): PublicKey {
  try {
 ...
 */
export function validateSolAmount(amount: string | number, fieldName: string = "amount"): number {
...
export function validateCapabilities(capabilities: string | number): number {
  const numCaps = typeof capabilities === "string" ? parseInt(capabilities, 10) : capabilities;
...
export function validateChannelName(name: string): string {
  ...
  if (!/^[a-zA-Z0-9_-]+$/.test(name)) {
    throw new ValidationError("Channel name can only contain letters, numbers, underscores, and hyphens");
  }
 ...
 */
export function validateFilePath(filePath: string, fieldName: string = "file"): string {
  if (!filePath.trim()) {
  ...
  if (filePath.includes("..") && !filePath.startsWith("~/")) {
    console.warn(chalk.yellow(`Warning: ${fieldName} path contains '..' - ensure this is intentional`));
  }
 ...
 */
export function validatePositiveInteger(value: string | number, fieldName: string = "value"): number {
