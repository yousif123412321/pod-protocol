...
const REQUIRED_SCRIPTS = ["build:prod", "test:coverage", "lint", "clean"];
...
function runProductionBuild() {
  ...
  try {
    // Clean and build SDK
    execSync("cd sdk && bun run clean && bun run build:prod", {
      stdio: "inherit",
    });

    // Clean and build CLI
    execSync("cd cli && bun run clean && bun run build:prod", {
      stdio: "inherit",
    });
...
function validateCLI() {
  ...
  try {
    ...
    try {
      execSync("node cli/dist/index.js --help", {
        encoding: "utf8",
        stdio: "pipe",
      });
    } catch (error) {
      ...
      if (
        ...
        !error.stdout?.includes("PoD Protocol CLI")
      ) {
    ...
    try {
      const statusOutput = execSync("node cli/dist/index.js status", {
        encoding: "utf8",
        stdio: "pipe",
      });
      ...
      log(
        "CLI status command executed (may require Solana network access)",
        "info",
      );
    }
...
async function main() {
  ...
  try {
    ...
    log(
      "The SDK and CLI packages are ready for production deployment.",
      "success",
    );
